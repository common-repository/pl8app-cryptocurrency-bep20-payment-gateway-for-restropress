...
var makeReadonly = function(cryptoId, numberOfSamples) {
  for (let i = 0; i < numberOfSamples; i++) {
    jQuery("#" + cryptoId + "_hd_mpk_sample_addresses-" + i).attr({
      readonly: "true"
    });
  }
};
...
var hideAddButton = function(cryptoId) {
  jQuery(
    "#pl8app_rpr_pro_redux_options-" +
      cryptoId +
      "_hd_mpk_sample_addresses .redux-multi-text-add"
  ).hide();
};

var getMpk = function(cryptoId) {
  return (mpk = jQuery("#" + cryptoId + "_hd_mpk-textarea")
    .val()
    .trim());
};
  ...
  xhr[cryptoId] = jQuery
    .ajax({
      ...
      beforeSend: function() {
        updateSampleText(
          cryptoId,
          numberOfSamples,
          "Generating HD Addresses..."
        );

        jQuery(
          "#pl8app_rpr_pro_redux_options-" +
            cryptoId +
            "_hd_mpk_sample_addresses input"
        ).removeClass("flash-red");
        jQuery(
          "#pl8app_rpr_pro_redux_options-" +
            cryptoId +
            "_hd_mpk_sample_addresses input"
        ).removeClass("flash-green");
        jQuery(
          "#pl8app_rpr_pro_redux_options-" +
            cryptoId +
            "_hd_mpk_sample_addresses input"
    ...
    .fail(function(response) {
      ...
      updateSampleText(
        cryptoId,
        numberOfSamples,
        "Address creation failed, please check your mpk."
      );
    ...
    .done(function(responseJson) {
      ...
      addresses = JSON.parse(responseJson).data;
      ...
      if (addresses[0] === "You have entered a valid Segwit MPK.") {
        ...
        if (
          jQuery("#" + cryptoId + "_hd_mpk_sample_addresses-0")
            .parent()
            .children().length === 2
        ) {
          jQuery("#" + cryptoId + "_hd_mpk_sample_addresses-0")
...
jQuery(document).ready(function() {
  ...
  hdCryptos.forEach(function(cryptoId) {
    ...
    firstAddressValue = jQuery(
      "#" + cryptoId + "_hd_mpk_sample_addresses-0"
    ).val();
    ...
    if (typeof this.getHdMode === "function") {
      var hdModeKey =
        'input[name="pl8app_rpr_pro_redux_options[' + cryptoId + '_hd_mode]"]';
